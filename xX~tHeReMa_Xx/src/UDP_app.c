...
//                    SYS_CONSOLE_MESSAGE("\r\nClient: Starting connection\r\n");
        ...
        case UDP_TCPIP_WAIT_FOR_CONNECTION:
        {
            if (!TCPIP_UDP_IsConnected(appData.clientSocket)) {
                SYS_CONSOLE_PRINT("Client: DEBUG1\r\n");
                break;
            }
            if (TCPIP_UDP_PutIsReady(appData.clientSocket) == 0) {
                SYS_CONSOLE_PRINT("Client: DEBUG2\r\n");
                break;
            }
...
//                    SYS_CONSOLE_PRINT("%s\r\n", UDP_Receive_Buffer);
//                if(prt_SWT_SWT7 == 1){
//                    SYS_CONSOLE_PRINT("%Index recu: %d\r\n", UDP_Receive_Buffer_Int[0]);
////                    SYS_CONSOLE_PRINT("longueure: %d\r\n",sizeof(UDP_Receive_Buffer));
...
//                    
//                }
//                    RGBLED_SetValue((UDP_Receive_Buffer_Int[39]>>8)%255,(UDP_Receive_Buffer_Int[79]>>8)%255,(UDP_Receive_Buffer_Int[119]>>8)%255);
//                }

//                strcpy(UDP_Receive_Buffer
//                    SYS_CONSOLE_PRINT("longuere : %d",sizeof(UDP_Receive_Buffer));
                memset(UDP_Receive_Buffer,0,sizeof(UDP_Receive_Buffer));
                memset(UDP_Receive_Buffer_Int,0,sizeof(UDP_Receive_Buffer_Int));
                appData.clientState = UDP_TCPIP_WAITING_FOR_COMMAND;
//                TCPIP_UDP_Close(appData.clientSocket);
            }
        }
